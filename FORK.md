...
1. **The published container image is frozen.** The `release.yml` workflow only
   ...
   critically, a dropped ZMQ peer left ingest wedged **forever**
...
That is precisely the failure the hardcoded `model3` caused. For the same
...
shared one bare `return` for two different cases: a malformed connectivity
payload, which breaks online/asleep/offline detection wholesale, and the benign
VIN-less case. The three `os.Setenv` calls in the HA add-on's MQTT autodetection
*are* that function's output, so a failure meant starting with no broker having
configured nothing wrong. `onboard` left both public-key writes unchecked, and
Tesla fetches that key to verify domain ownership.
...
the config. That copy is the one that leaks, and because deleting it crash-loops
the gateway on a validation error, it is also the copy nobody dares delete.
Validation now accepts either the seed or a non-empty cache file.
...
bug reports. `redactQuery` keeps the useful part, which parameters were sent,
...
destination and the odometer. That is a live location feed for a named car,
previously unauthenticated with nothing in front of it but whatever address the
operator bound to.
...
already receives, with no extra stream config and no API call.
...
The gateway registers a Last Will on `<state_topic_base>/availability` and
every discovery config, curated, generic and command, points at it, so when the
...
lies. Verified both ways against the broker: `docker compose stop` to `offline`,
`docker kill` to `offline`, start to `online`.
...
That is precisely what makes an asleep car's sensors read `unknown` after a
...
sent at all, so slow-moving values are *missing* from the emulated document
rather than merely stale: `charge_limit_soc` and `sentry_mode` before their first
change, `trim_badging`, a parked `odometer`. `Relay.VehicleData` fetches one real
...
- **The endpoint list must be percent-encoded.** Sent with literal semicolons,
  ...
  car reporting nothing. That is a silent failure that produces a seed missing
  exactly the values the seed exists for. Measured, then fixed and tested.
...
them, whether from a fresh persistence database, a replaced container, or
retained messages cleared by hand, the curated entities returned on the next
reconnect, because the connect handler clears `announced`. The generic per-field
entities did not: `discovered` was never cleared, so they were published exactly
once per process and a diagnostic entity could stay missing from Home Assistant
until someone happened to restart the gateway. One failure, two very different
recovery times, for no reason a user could see.
...
`Stopped` false. DC power is only believed above the floor.
...
The relay's `tokenManager` already had the better implementation, with a mutex,
a cache and persist-on-rotation, so it became the owner rather than being
duplicated. `Relay.AccessToken()` and `Relay.SetRefreshToken()` are the whole
surface, passed to the wizard as `onboard.TokenOwner` so `onboard` does not
import `commands`. With no relay (commands disabled) the wizard still refreshes
for itself, and now persists the replacement.
...
build is deployed. The image was built, but the container is still serving the
previous one. That is half of why that gate exists.
